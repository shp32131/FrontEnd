...
> 使用\<script\>标签的src属性引入外部文件时则标签中的js代码不会执行  
```html
...
### 避免延迟
> 在html文档中，如果\<head\>标签中有js代码，则要等到\<head\>标签中的js代码加载并解析完成后才会渲染显示\<body>标签中的内容，所以一般js代码放在\<body\>中  

### 弱类型
> javascript是弱类型脚本语言，变量的类型由其所引用的值决定 
```javascript
let test = "hello";
...
### 变量提升
> 解析器先解析代码把变量的声明提升到最前面,就叫变量的提升  
>
> - 使用var定义声明的变量，变量的声明会提升到最前面，赋值还在原位置  
```javascript
console.log(test);
var test = 'hello';
//以上代码解析执行过程如下
...
### TDZ 变量暂时死区
> TDZ 称暂时性死区，指变量在作用域内已经存在，但必须在let/const声明后才可以使用  
> - 可以让程序保持先声明后使用的习惯，让程序更稳定  
> - 使用let/const声明的变量,在声明前存在TDZ,使用时会发生错误  
> - 变量要先声明后使用,建议使用let/const，而少使用var  
```javascript
console.log(x);//cannot access 'x' before initialization
let x = 10;
//在函数作用域中产生TDZ，不允许变量未声明前使用
var a = 20;
function run(){
    console.log(a);
    let a = 30;
...
#### var
> var声明的变量存在于最近的函数或全局作用域中  
> - var会发生变量提升  
> - var有函数作用域，但没有块作用域容易污染全局
```javascript
//var,let,const都有函数作用域
function test()
{
    web = "hello";//未使用关键字声明，默认为全局变量  
    var a = 100;
}
//块作用域  
{
...
test();
console.log(web);//hello,全局变量
console.log(a);//undefined,var有函数作用域 
console.log(b);//1,var声明的变量无块作用域 
console.log(c);//ReferenceError:not defined,有块作用域 
console.log(d);//ReferenceError:no defined,有块作用域 
//上面函数中如果改成var/let/const web = 'hello';就不会污染全局变量了
```
...
> 在do/while语句中使用var声明的变量可以在语句外部访问到 
```javascript
var num = 0;
function show() {
  var step = 10;
...
> 在html文档中,var全局声明的变量也存在于window对象中  
```javascript
var a = 10;
console.log(window.a);//10
```
> es6之前没有let/const时，使用立即执行函数模拟var块作用域,避免变量污染   
```javascript
(function(str){
    return str;
})('hello')
...
#### 立即执行函数 
> 主要为了隔离作用域，避免变量污染,使函数立即执行有两个条件 :
> - 函数体必须是函数表达式,不能是函数声明,函数体上有运算符    
> - 函数体后面要有小括号()   
> - 运算符的作用是将匿名函数或函数声明转换为函数表达式
> - 作用在函数体上的运算符可以是 (),！，+，-,=  
...
#### let <sup>es6</sup>
> let与var声明变量的区别是:
> - let/const没有变量提升特性,let/const有TDZ暂时死区  
> - let/const不允许在同一作用域内重复声明同一个变量  
> - let/const有块作用域
```javascript
let i = 20;
for(let i =0;i < 3;i++)
...
> - 和let一样拥有块，函数，全局作用域
> - 常量名建议全部大写
> - const声明常量的本质是变量所指向的内存空间存储的数据(栈数据)不允许改变   
> - const声明的基本数据类型是一个常量，但引用类型的实际值(堆数据)不受影响可以改变   
> 
> js基本数据类型和引用数据类型存储值方式是不一样的:  
> - 基本数据(number,string,boolean),实际值保存在内存中,是栈数据     
> - 复合引用类型数据(object,array,function...),实际值保存在硬盘中，是堆数据，内存中保存的是实际值的指针   
...
#### 重复定义
> 同一作用域，使用var可能多次声明赋值，但let\/const不允许在同一作用域重复声明  

#### Object.freeze
> 使用Object.freeze()冻结变量后，变量不可以修改了，如果是严格模式("use strict";)，则会报错  
...
> 基本数据传值，引用数据传址 
> 
> - 基本数据类型有number,string,boolean,symbol,null,undefined   
> - 引用类型型有function,object,set,map,weakSet,weakMap    
> - 基本类型赋值是值的复制，互相不影响
...
### undefined
> 对声明但未赋值的变量返回类型为undefined,表示值未定义   
> 
> - 对未声明的变量使用会报错(ReferenceError:not defined)  
> - 使用typeof判断类型时显示undefined  
> - 未赋值与未声明定义的变量值都为undefined,所以声明变量应该设置一个初始值，这样可以区分变量状态  
```javascript
...
### null
> null用于定义一个空对象，即如果一个变量要用来保存引用类型，则可以用null初始化这个变量  
...
#### for/in
> 用于遍历对象所有的属性，for/in主要用于遍历对象，不建议用来遍历数组 
>
> - 对象不是迭代器，数组默认就是迭代器  
```javascript
let web = {
...
> 用来遍历Arrays,Strings,Maps,Sets等可迭代的数据结构   
> - 对象可通过Object.keys(),Object.values(),Object.entries()转为可迭代数据结构  
> - for/of直接遍历数组值value,for/in遍历对象的key  
```javascript
let arr = [1,2,3];
...
```
> 只有对象才有方法调用，但在也可以使用值类型调用方法，它会在执行时将值转换为对象  
```javascript

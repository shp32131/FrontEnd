...
```
> 特点：最简单的排序算法
- 最快情况：当数组已经是排好的顺序时(就是if条件一直不满足)O(n)  
- 最坏情况: 当数组是反序时 O(n^2)
...
```
> 特点：最好最坏都是 O(n^2),不占用额外空间，数据规模越小越好   
...
。既可以提前设定好间隔序列，也可以动态的定义间隔序列。动态定义间隔序列的算法是《算法（第4版》的合著者Robert Sedgewick提出的。在这里，我就使用了这种方法。
